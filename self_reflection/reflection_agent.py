...
refine_prompt = ChatPromptTemplate.from_messages([
    ('system', '''你是一位技术写作专家。请根据评审意见改进以下文章。
要求:
1. 保留原文的优点
2. 针对评审意见中指出的问题进行改进
3. 控制在200字以内
...
for iteration in range(1, MAX_ITERATIONS + 1):
    ...
    if current_score >= THRESHOLD_SCORE:
        print(f'[轮次{iteration}] 分数 {current_score} >= {THRESHOLD_SCORE}，质量达标! 停止迭代。')
        break
...
def should_continue(state: ReflectionState) -> str:
    ...
    if score >= THRESHOLD_SCORE:
        ...
        print(f'  [Router] 分数 {score} < {THRESHOLD_SCORE}，轮次 {iteration}/{max_iter} → refine')
        return 'refine'
...
if len(final_state['history']) > 1:
    ...
    for i, version in enumerate(final_state['history']):
        ...
        print(f'  [{label}] {version[:80]}...' if len(version) > 80 else f'  [{label}] {version}')
    print()
...
code_generate_prompt = ChatPromptTemplate.from_messages([
    ('system', '''你是一位Python开发者。请根据需求编写代码。
...
code_review_prompt = ChatPromptTemplate.from_messages([
    ('system', '''你是一位资深Python代码审查专家。请从以下维度审查代码:
...
code_refine_prompt = ChatPromptTemplate.from_messages([
    ('system', '''你是一位Python开发者。请根据代码审查意见改进代码。
要求:
1. 保留原有功能
2. 针对审查意见进行改进
...
for code_iter in range(1, CODE_MAX_ITERATIONS + 1):
    ...
    if code_score >= CODE_THRESHOLD:
        print(f'[轮次{code_iter}] 代码质量达标 (分数 {code_score} >= {CODE_THRESHOLD})，停止迭代。')
        break
