...
print(
    f"[无缓存] 每日成本: {daily_questions} 次 x {cost_per_call}元 = {no_cache_cost:.2f}元"
)
print(
    f"[无缓存] 每日总延迟: {daily_questions} 次 x {latency_per_call}秒 = {no_cache_time:.0f}秒"
)
...
print(
    f"[有缓存] 每日成本: {unique_questions:.0f} x {cost_per_call}元 = {with_cache_cost:.2f}元"
)
...
print()
print(
    f"[节省] 成本节省: {savings_pct:.0f}%  ({no_cache_cost:.2f}元 → {with_cache_cost:.2f}元)"
)
print(f"[节省] 这就是缓存的威力!")
...
cache_entries = [
    ...
    (
        "如何学习机器学习",
        "学习机器学习建议从数学基础开始，然后学习经典算法，最后做实战项目。",
    ),
]
...
if result_2["hit"]:
    ...
    print(
        f"  [精彩] 虽然文字不同，但语义相似度 {result_2['similarity']:.4f} > 阈值 0.85，命中!"
    )
else:
...
total_tokens_with_cache = (
    daily_requests * (1 - cache_hit_rate) * (avg_input_tokens + avg_output_tokens)
)
