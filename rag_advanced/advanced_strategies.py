...
print('  BM25 索引构建完成 (文档数=%d, 平均文档长度=%.0f tokens)' % (
...
def crag_pipeline(query, k=3):
    ...
    if len(relevant_docs) >= 1:
        # 有相关文档，直接生成
        print('    [CRAG] Step 3: 找到 %d 篇相关文档，直接生成答案' % len(relevant_docs))
...
def self_rag_pipeline(query, max_retries=2):
    ...
    for attempt in range(max_retries + 1):
        print('    [Self-RAG] Step 3: 生成答案 (尝试 %d/%d)...' % (
