...
for q in test_questions:
    ...
    print('\n  【结果对比】:')
    print('    Naive (单次检索): 召回 %d 篇，章节=%s' % (
        len(naive_docs),
        [d.metadata.get('chapter', '') for d in naive_docs]
    ))
    print('    Multi-Query (合并去重): 召回 %d 篇，章节=%s' % (
        len(merged_docs),
        [d.metadata.get('chapter', '') for d in merged_docs]
    ))
    print('    召回率提升: %d → %d 篇 (%.0f%% ↑)' % (
        len(naive_docs),
        len(merged_docs),
...
for q in pc_test_questions:
    ...
    print('    - Naive 上下文: %d字 | Parent-Child 上下文: %d字' % (
