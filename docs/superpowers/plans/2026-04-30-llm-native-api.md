# 项目零：LLM 原生 API 核心调用 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 重构项目目录结构，新建 `llm/native_api.py` 教学文件，用纯原生 OpenAI SDK 演示 Messages 三角色、阻塞式调用数据包解剖、流式打字机效果，不使用任何 LangChain 组件。

**Architecture:** 单文件渐进式教学，4个章节（文件头科普 + 第0/1/2章），从零到完整依次运行。同时将现有 `chatbot.py` 移入 `langchain/` 子目录，形成 `llm/` vs `langchain/` 的清晰分层。

**Tech Stack:** Python 3.9+, openai>=1.0.0（已在 requirements.txt），内部 chj.cloud LLM Gateway（OpenAI 兼容接口）
...
| 操作 | 路径 | 职责 |
|------|------|------|
| Create dir | `llm/` | 纯原生 OpenAI SDK 学习系列 |
| Create | `llm/native_api.py` | 项目零主教学文件（本次任务） |
| Create dir | `langchain/` | LangChain 框架学习系列 |
| Move | `chatbot.py` → `langchain/chatbot.py` | 项目一（已完成，搬家） |
| Keep | `requirements.txt` | 根目录共用，openai>=1.0.0 已包含 |
...
**Files:**
- Create dir: `llm/`
- Create dir: `langchain/`
- Move: `chatbot.py` → `langchain/chatbot.py`

- [ ] **Step 1：创建两个子目录**

```bash
mkdir -p /Users/liuyu22/Desktop/langchain_learning/llm
mkdir -p /Users/liuyu22/Desktop/langchain_learning/langchain
```

预期：命令无输出，两个目录创建成功。

- [ ] **Step 2：用 git mv 移动 chatbot.py（保留 git 历史）**

```bash
cd /Users/liuyu22/Desktop/langchain_learning
git mv chatbot.py langchain/chatbot.py
```

预期：命令无输出。`chatbot.py` 从根目录消失，出现在 `langchain/chatbot.py`。

- [ ] **Step 3：验证移动结果**

```bash
ls /Users/liuyu22/Desktop/langchain_learning/
ls /Users/liuyu22/Desktop/langchain_learning/langchain/
```

预期输出（根目录不再有 chatbot.py）：
```
docs/  langchain/  llm/  requirements.txt  ...
```
```
chatbot.py
```

- [ ] **Step 4：验证 langchain/chatbot.py 可正常运行**
...
预期：前几章的输出正常打印（和搬家前一致），第4章进入交互聊天循环后输入 `quit` 退出。

- [ ] **Step 5：提交目录重构**

```bash
git add langchain/chatbot.py
git commit -m "refactor: move chatbot.py into langchain/ subdirectory

Restructure project to separate native LLM API (llm/) from
LangChain framework examples (langchain/).
...
## Task 2：创建 `llm/native_api.py` — 文件头科普 + 第 0 章

**Files:**
- Create: `llm/native_api.py`

- [ ] **Step 1：写入文件头 docstring 和第 0 章**

创建 `/Users/liuyu22/Desktop/langchain_learning/llm/native_api.py`，内容如下：
...
✅ OpenAI 原生客户端初始化完成
   模型: kivy-kimi-k2_5
   接口: https://llm-gateway-proxy.inner.chj.cloud/llm-gateway/v1
```

- [ ] **Step 3：提交第 0 章**

```bash
git add llm/native_api.py
git commit -m "feat: add project zero - llm/native_api.py chapter 0 (init)

File header with 剧组拍戏 analogy for System/User/Assistant roles
and Temperature explanation. Chapter 0: native OpenAI client setup.
...
## Task 3：追加第 1 章（阻塞式调用 + 数据包解剖）

**Files:**
- Modify: `llm/native_api.py`（在文件末尾追加）

- [ ] **Step 1：追加第 1 章代码**

在 `llm/native_api.py` 末尾追加：
...
【发送给 API 的 messages 数组】
  [0] role='system'      content='你是一个严谨的科普作家，用通俗易懂的语言解释复杂概念，回答控制在60'...
  [1] role='user'        content='黑洞是什么？'...

【正在调用 API（阻塞中，请稍候...）】

【API 返回的完整原始对象（这就是 OpenAI 服务器发回来的数据）】
------------------------------------------------------------
ChatCompletion(id='...', choices=[Choice(finish_reason='stop', ...)], ...)
------------------------------------------------------------

【逐层解剖关键字段】

  ① response.choices  （类型: <class 'list'>，长度: 1）
     → choices 是一个列表，每个元素是一个候选回复（通常只有 1 个）

  ② response.choices[0].message  （类型: ChatCompletionMessage）
     → role: 'assistant'
     → content（AI 实际说的话）：
       【黑洞是...】

  ③ response.choices[0].finish_reason: 'stop'
     → 'stop' = AI 自然结束  'length' = 被 max_tokens 截断

  ④ response.usage（Token 消耗统计）：
     → prompt_tokens（你的输入）:    xx
     → completion_tokens（AI 输出）: xx
     → total_tokens（本次总消耗）:   xx

💡 小结：LangChain 的 StrOutputParser 做的事，等价于：
...
```bash
git add llm/native_api.py
git commit -m "feat: add llm/native_api.py chapter 1 - blocking call + response dissection

Demonstrates raw ChatCompletion object structure: choices,
finish_reason, usage (prompt/completion/total tokens).
...
【流式打字机效果（注意：下面的字会一个个蹦出来）】
AI：天空之所以是蓝色的... （文字逐字蹦出，像打字机）

【流式接收完毕！完整回复共 xxx 个字符】

💡 小结：流式 vs 阻塞式，代码差异只有两处：
   1. create(..., stream=True)
   2. for chunk in stream:  →  chunk.choices[0].delta.content
      （delta = 增量碎片，而非 message = 完整文本）

============================================================
🎉 项目零学习完毕！你已经掌握了 LLM 最底层的调用方式。
   下一站：langchain/chatbot.py —— 看看 LangChain 帮你封装了什么。
============================================================
```

关键验证点：文字输出时应明显有"逐字出现"的动态效果，而不是一次性打印。

- [ ] **Step 3：提交第 2 章，完成项目零**

```bash
git add llm/native_api.py
git commit -m "feat: add llm/native_api.py chapter 2 - streaming typewriter effect

Demonstrates stream=True with for-chunk-in-stream loop,
delta.content vs message.content, and flush=True for real-time output.
...
- [x] Task 1 覆盖目录重构（llm/ + langchain/ 创建，chatbot.py 移入）
- [x] Task 2 覆盖文件头科普（剧组比喻 + Temperature）和第0章初始化
- [x] Task 3 覆盖第1章：阻塞式调用 + choices/finish_reason/usage 解剖
- [x] Task 4 覆盖第2章：stream=True + for chunk + delta.content + flush=True
- [x] 每个 Step 都有完整可直接使用的代码，无 TBD 占位符
- [x] 每个 Step 都有预期输出，方便验证
- [x] API 凭据与 langchain/chatbot.py 保持一致（共用已验证接口）
- [x] 从不使用 langchain 相关 import（只有 `from openai import OpenAI`）
- [x] delta.content vs message.content 区别有明确注释
- [x] flush=True 的必要性有明确注释
