# LangChain 教学聊天机器人 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 `~/Desktop/langchain_learning/` 下创建一个带历史记忆的控制台聊天机器人，通过保姆级注释教会新手 LangChain 核心组件。

**Architecture:** 单文件渐进式教学，5个章节从零到完整，每章独立可运行演示片段，最终第4章汇总为完整 while 循环聊天机器人。LLM 接入理想内部 lixiang.com DeepSeek-V3 接口，通过 `ChatOpenAI(base_url=...)` 对接。

**Tech Stack:** Python 3.10+, langchain>=0.3.0, langchain-openai>=0.2.0, ChatOpenAI, RunnableWithMessageHistory, ChatMessageHistory, StrOutputParser, ChatPromptTemplate
...
| 操作 | 路径 | 职责 |
|------|------|------|
| Create | `~/Desktop/langchain_learning/requirements.txt` | 项目依赖 |
| Create | `~/Desktop/langchain_learning/chatbot.py` | 主教学文件（5章节渐进式） |
...
**Files:**
- Create: `~/Desktop/langchain_learning/requirements.txt`
...
- [ ] **Step 2: 创建并激活 venv，安装依赖**

```bash
cd ~/Desktop/langchain_learning
python -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
```

预期输出：`Successfully installed langchain-... langchain-openai-... openai-...`

- [ ] **Step 3: 验证安装**

```bash
python -c "import langchain; import langchain_openai; print('安装成功，LangChain 版本:', langchain.__version__)"
```

预期输出：`安装成功，LangChain 版本: 0.3.x`

---

### Task 2: 编写 chatbot.py — 文件头部说明 + 第0章（环境初始化）

**Files:**
- Create: `~/Desktop/langchain_learning/chatbot.py`
...
✅ LLM 初始化完成
   模型: deepseek-v3-0324
   接口: https://lpai-llm.lixiang.com/inference/deepseek-ai/deepseek-v3/v1/
```

---

### Task 3: 编写第1章（PromptTemplate）

**Files:**
- Modify: `~/Desktop/langchain_learning/chatbot.py`（在第0章末尾追加）
...
```
============================================================
第 1 章：PromptTemplate 演示
============================================================
【模板原始结构】
...
【填充变量后的提示词（LLM 将收到这个）】
  [SYSTEM] 你是一位专业的Python 编程领域专家，请用通俗易懂的语言回答问题。
  [HUMAN] 什么是列表推导式？
```

---

### Task 4: 编写第2章（LCEL 管道）+ 第3章（OutputParser）

**Files:**
- Modify: `~/Desktop/langchain_learning/chatbot.py`（继续追加）
...
预期新增输出（内容因 AI 回答而异，但结构固定）：
```
============================================================
第 2 章：LCEL 管道演示
============================================================
【LCEL 链已创建】
  链的结构：demo_prompt | llm | parser
...
============================================================
第 3 章：OutputParser 对比演示
============================================================
【不带 OutputParser 的原始输出】
  类型: <class 'langchain_core.messages.ai.AIMessage'>
  内容: content='...' ...
【带 StrOutputParser 的解析后输出】
  类型: <class 'str'>
  内容: ...
💡 结论：StrOutputParser 把 AIMessage 对象剥开，只留下 .content 文本
```

---

### Task 5: 编写第4章（带记忆的完整聊天机器人）
...
你：我叫小明，你好
你：我刚才说我叫什么名字？
你：我喜欢学 Python，你有什么建议？
你：根据我的爱好，帮我推荐一本书
你：quit
```

预期第2轮时 AI 能正确回答"你叫小明"，证明记忆生效。
每轮对话后应看到：`💾 [记忆状态] 当前会话共存储 N 条消息 (N/2 轮对话)`

---

## 自检清单（Spec Self-Review）

- [x] 每个 Task 都有具体的文件路径（无"TBD"）
- [x] 每个 Step 都有完整代码，无占位符
- [x] 覆盖所有需求：PromptTemplate / LCEL / OutputParser / RunnableWithMessageHistory
- [x] 使用最新 API（无 LLMChain / ConversationBufferMemory 等废弃组件）
- [x] 包含充足 print() 语句观察运行状态
- [x] 避坑注释标注了新老版本差异
- [x] requirements.txt 已包含
- [x] 使用理想内部 lixiang.com 接口
