...
def step_one(state: ChatState) -> dict:
    ...
    return {
        'messages': [f'[Step {count}] 你好! 我是步骤一的输出'],
        'step_count': count
    }
...
def step_two(state: ChatState) -> dict:
    ...
    count = state.get('step_count', 0) + 1
    return {
        'messages': [f'[Step {count}] 我是步骤二, 处理完毕!'],
        'step_count': count
    }
...
result_1 = ckpt_graph.invoke(
    {'messages': ['用户: 第一次对话开始'], 'step_count': 0},
    config=config_1
)
...
result_2 = ckpt_graph.invoke(
    {'messages': ['用户: 第二次对话继续'], 'step_count': 0},
    config=config_1
)
...
result_3 = ckpt_graph.invoke(
    {'messages': ['用户: 这是一个全新的对话'], 'step_count': 0},
    config=config_2
)
...
print(f'  验证 state.values["human_approved"]: {updated_state.values["human_approved"]}')
...
def draft_node(state: WritingState) -> dict:
    ...
    if revision_count == 0:
        ...
        response = llm.invoke([
            SystemMessage(content='你是一个专业的文案写手。请根据主题写一段简短的宣传文案(50字以内)。'),
        ...
        response = llm.invoke([
            SystemMessage(content='你是一个专业的文案写手。请根据反馈修改文案(50字以内)。'),
...
def revise_node(state: WritingState) -> dict:
    ...
    response = llm.invoke([
        SystemMessage(content='你是一个专业的文案写手。请根据反馈修改文案(50字以内)。直接输出修改后的文案。'),
        HumanMessage(content=f'原文案: {old_draft}\n\n修改要求: {feedback}')
    ])
